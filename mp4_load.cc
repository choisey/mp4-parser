...
MP4Load::MP4LoadVisitor::~MP4LoadVisitor()
{
	assert( !_f->is_open() );
}

bool MP4Load::MP4LoadVisitor::readI32(int& i32)
{
	uint8_t buf[4];
...
bool MP4Load::MP4LoadVisitor::readU8(uint8_t& u8)
{
	uint8_t buf[1];
	if ( _f->read(buf, sizeof(buf)) == sizeof(buf) ) {
		u8 = buf[0];
...
bool MP4Load::MP4LoadVisitor::readU32(uint32_t& u32)
{
	uint8_t buf[4];
	if ( _f->read(buf, sizeof(buf)) == sizeof(buf) ) {
...
bool MP4Load::MP4LoadVisitor::readU64(uint64_t& u64)
{
	uint8_t buf[8];
	if ( _f->read(buf, sizeof(buf)) == sizeof(buf) ) {
...
bool MP4Load::MP4LoadVisitor::readBoxHead(BoxHead& boxhead)
{
	assert( _f->is_open() );
...
//	if ( boxsize <= sizeof(boxsize) || !readU32(_fp, boxtype) ) {
		...
		case DREF:
			{
				...
				for ( uint32_t i = 0; i < entry_count; i++ ) {
					size_t nb = readBox(offset, head.boxtype, boxes);
					if ( 0 == nb ) {
						break;
					}
...
void MP4Load::MP4LoadVisitor::visit(BoxHead& head, FileTypeBox& ftyp)
{
...
void MP4Load::MP4LoadVisitor::visit(BoxHead& head, MovieHeaderBox& mvhd)
{
	if ( 1 == head.version ) {
...
void MP4Load::MP4LoadVisitor::visit(BoxHead& head, MovieExtendsHeaderBox& mehd)
{
	if ( 1 == head.version ) {
		uint8_t buf[8];
		if ( _f->read(buf, sizeof(buf)) != sizeof(buf) ) {
...
void MP4Load::MP4LoadVisitor::visit(BoxHead& head, TrackHeaderBox& tkhd)
{
	if ( 1 == head.version ) {
...
void MP4Load::MP4LoadVisitor::visit(BoxHead& head, MediaHeaderBox& mdhd)
{
	if ( 1 == head.version ) {
...
void MP4Load::MP4LoadVisitor::visit(BoxHead& head, HandlerBox& hdlr)
{
...
void MP4Load::MP4LoadVisitor::visit(BoxHead& head, VideoMediaHeaderBox& vmhd)
{
	uint8_t buf[8];
	if ( _f->read(buf, sizeof(buf)) != sizeof(buf) ) {
		return;
...
void MP4Load::MP4LoadVisitor::visit(BoxHead& head, SoundMediaHeaderBox& smhd)
{
	uint8_t buf[4];
	if ( _f->read(buf, sizeof(buf)) != sizeof(buf) ) {
		return;
...
void MP4Load::MP4LoadVisitor::visit(BoxHead& head, HintMediaHeaderBox& hmhd)
{
	uint8_t buf[16];
	if ( _f->read(buf, sizeof(buf)) != sizeof(buf) ) {
		return;
...
//void MP4Load::MP4LoadVisitor::visit(BoxHead& head, SampleDescriptionBox::VisualSampleEntry& sd)
//{
//	uint8_t buf[78];
...
//}
//
//void MP4Load::MP4LoadVisitor::visit(BoxHead& head, SampleDescriptionBox::AudioSampleEntry& sd)
//{
//	uint8_t buf[28];
//	if ( 1 != fread(buf, sizeof(buf), 1, _fp) ) {
//		return;
...
//}
//
//void MP4Load::MP4LoadVisitor::visit(BoxHead& head, SampleDescriptionBox::HintSampleEntry& sd)
//{
//	size_t size = head.boxsize - head.boxheadsize;
//	uint8_t buf[size];
//	if ( 1 != fread(buf, sizeof(buf), 1, _fp) ) {
...
void MP4Load::MP4LoadVisitor::visit(BoxHead& head, TimeToSampleBox& stts)
{
	uint32_t entry_count;
	if ( !readU32(entry_count) ) {
		return;
...
void MP4Load::MP4LoadVisitor::visit(BoxHead& head, CompositionOffsetBox& ctts)
{
	uint32_t entry_count;
	if ( !readU32(entry_count) ) {
		return;
...
void MP4Load::MP4LoadVisitor::visit(BoxHead& head, SampleToChunkBox& stsc)
{
	uint32_t entry_count;
	if ( !readU32(entry_count) ) {
		return;
...
void MP4Load::MP4LoadVisitor::visit(BoxHead& head, SampleSizeBox& stsz)
{
	uint8_t buf[8];
	if ( _f->read(buf, sizeof(buf)) != sizeof(buf) ) {
		return;
...
void MP4Load::MP4LoadVisitor::visit(BoxHead& head, ChunkOffsetBox& stco)
{
	uint32_t entry_count;
	if ( !readU32(entry_count) ) {
		return;
...
void MP4Load::MP4LoadVisitor::visit(BoxHead& head, ChunkLargeOffsetBox& co64)
{
	uint32_t entry_count;
	if ( !readU32(entry_count) ) {
		return;
...
void MP4Load::MP4LoadVisitor::visit(BoxHead& head, SyncSampleBox& stss)
{
	uint32_t entry_count;
	if ( !readU32(entry_count) ) {
		return;
...
void MP4Load::MP4LoadVisitor::visit(BoxHead& head, SampleDependencyTypeBox& sdtp)
{
	uint32_t entry_count = ( head.boxsize - head.boxheadsize ) / sizeof(uint8_t);;

	for ( uint32_t i = 0; i < entry_count; i++ ) {
		uint8_t sample_dependency;
		if ( !readU8(sample_dependency) ) {
			return;
		}
...
void MP4Load::MP4LoadVisitor::visit(BoxHead& head, EditListBox& elst)
{
	uint32_t entry_count;
	if ( !readU32(entry_count) ) {
		return;
...
/*
void MP4Load::MP4LoadVisitor::visit(BoxHead& head, DataReferenceBox& dref)
{
	uint32_t entry_count;
	if ( !readU32(entry_count) ) {
		return;
...
void MP4Load::MP4LoadVisitor::visit(BoxHead& head, TrackFragmentHeaderBox& tfhd)
{
	...
	if ( 0x000001 & head.flag ) {
		if ( !readU64(tfhd.base_data_offset) ) {
			return;
		}
	}

	if ( 0x000002 & head.flag ) {
		if ( !readU32(tfhd.sample_description_index) ) {
			return;
		}
	}

	if ( 0x000008 & head.flag ) {
		if ( !readU32(tfhd.default_sample_duration) ) {
			return;
		}
	}

	if ( 0x000010 & head.flag ) {
		if ( !readU32(tfhd.default_sample_size) ) {
			return;
		}
	}

	if ( 0x000020 & head.flag ) {
		if ( !readU32(tfhd.default_sample_flags) ) {
			return;
		}
	}
}

void MP4Load::MP4LoadVisitor::visit(BoxHead& head, TrackRunBox& trun)
{
	...
	if ( 0x000001 & head.flag ) {
		if ( !readI32(trun.data_offset) ) {
			return;
		}
	}

	if ( 0x000004 & head.flag ) {
		if ( !readU32(trun.first_sample_flags) ) {
			return;
		}
	}
...
void MP4Load::MP4LoadVisitor::visit(BoxHead& head, SegmentIndexBox& sidx)
{
	uint32_t entry_count;
	if ( 1 == head.version ) {
		uint8_t buf[28];
...
void MP4Load::MP4LoadVisitor::visit(BoxHead& head, MovieFragmentHeaderBox& mfhd)
{
...
void MP4Load::MP4LoadVisitor::visit(BoxHead& head, TrackFragmentDecodeTimeBox& tfdt)
{
...
void MP4Load::MP4LoadVisitor::visit(BoxHead& head, TrackFragmentRandomAccessBox& tfra)
{
...
void MP4Load::MP4LoadVisitor::visit(BoxHead& head, MovieFragmentRandomAccessOffsetBox& mfro)
{
...
void MP4Load::MP4LoadVisitor::visit(BoxHead& head, TrackExtendsBox& trex)
{
	uint8_t buf[20];
	if ( _f->read(buf, sizeof(buf)) != sizeof(buf) ) {
		return;
...
void MP4Load::MP4LoadVisitor::visit(BoxHead& head, MediaDataBox& mdat)
{
	mdat.uri = _f->uri();
...
void MP4Load::MP4LoadVisitor::visit(BoxHead& head, DataBox<std::vector<uint8_t>>& box)
{
	size_t size = head.boxsize - head.boxheadsize;
	if ( 0 < size ) {
		uint8_t buf[size];
...
void MP4Load::MP4LoadVisitor::visit(BoxHead& head, EmptyBox&)
{
}

// MP4Load

MP4Load::MP4Load(const char* uri)
	: _uri(uri)
{
}
...
void MP4Load::execute(std::vector<std::shared_ptr<MP4File>>& mp4files)
{
